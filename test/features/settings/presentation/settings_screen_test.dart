...
void main() {
  ...
  testWidgets('data action failure displays fallback message', (tester) async {
    await tester.pumpWidget(
      buildSubject(onExportData: () async => throw Exception('disk full')),
    );
  ...
  testWidgets('typed data action failure displays provided message', (
    tester,
  ) async {
    await tester.pumpWidget(
      buildSubject(
        onExportData: () async =>
            throw const SettingsDataActionFailure('Sharing unavailable.'),
      ),
    );
